...
<form class="w3-container" method="post" action="ListIndex.php" <?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>>
    <div class="TaskAddForm">
        <label class="w3-text-red"><b>task</b></label>
        <input class="w3-input w3-border w3-light-grey" type="text" name="task_name" value="<?= $_POST["task_name"] ?>">
        <span class="error"> <?php echo $task_nameErr; ?></span>

        <label class="w3-text-red"><b>duration task</b></label>
        <input class="w3-input w3-border w3-light-grey" type="number" name="task_duration" value="<?= $_POST["task_duration"] ?>">
        <span class="error"> <?php echo $task_durationErr; ?></span>

        <select class="mdb-select md-form" name="task_status" value="<?= $_POST["task_status"] ?>">
